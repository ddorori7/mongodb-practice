...
//  body-parser 등록
//  4.16 버전 이후에는 express 내부에 bodyParser 가 포함
...
app.get("/", (req, resp) => {
  ...
  /*
    ...
    resp.end();
    */
  resp.status(200).contentType("text/html;charset=utf-8").render("home");
});
...
app.get("/render", (req, resp) => {
  //  응답객체의 render 메서드 활용
  resp.contentType("text/html;charset=utf-8").render("render");
});
...
function startServer() {
  ...
  MongoClient.connect(dburl, { useNewUrlParser: true })
    .then((client) => {
      ...
      //  익스프레스에 추가
      app.set("db", db); // db 키로 몽고db 클라이언트 추가
